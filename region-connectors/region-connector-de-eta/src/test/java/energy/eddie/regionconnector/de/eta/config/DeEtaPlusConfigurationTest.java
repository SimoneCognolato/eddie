 ...
 * Verifies that the record constructor assigns all fields correctly
 * and that Spring property binding applies {@code @DefaultValue} defaults
 * when optional properties are omitted.
 */
    ...
    void springBinding_onlyRequiredProperties_defaultsAppliedForOptional() {
        contextRunner
                ...
                .run(context -> {
                    assertThat(context).hasSingleBean(DeEtaPlusConfiguration.class);
                    DeEtaPlusConfiguration config = context.getBean(DeEtaPlusConfiguration.class);
                    assertThat(config.apiBaseUrl()).isEqualTo("https://eta-plus.com");
                    assertThat(config.meteredDataEndpoint()).isEqualTo("/api/meters/historical");
