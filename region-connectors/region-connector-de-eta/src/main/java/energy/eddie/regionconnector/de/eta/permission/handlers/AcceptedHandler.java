    ...
    public void accept(AcceptedEvent event) {
        var pr = repository.findByPermissionId(event.permissionId())
                .orElseThrow(() -> {
                    ...
                    return new PermissionNotFoundException(event.permissionId());
                });
    ...
    private void handleError(Throwable error, String permissionId) {
        if (error instanceof RateLimitException) {
            LOGGER.atWarn()
                  ...
                  .log("Rate limit exceeded for permission request {}, retry with backoff recommended");
            throw (RateLimitException) error;
        } else if (error instanceof HttpClientErrorException.Forbidden) {
            LOGGER.atWarn()
                  .addArgument(permissionId)
            ...
            try {
                ...
                throw new OutboxPublishingException(
                        "Failed to persist REVOKED event for permission " + permissionId, ex
                );
            }
        } else {
            LOGGER.atError()
